...
$("#reminder-form").onsubmit=e=>{
  ...
  if(editingId) reminders=reminders.map(r=>r.id===editingId?{...r,...data,notified:false}:r);
  else reminders.push({id:Date.now(),...data,done:false,notified:false});
  saveStore(); closeModal(); render(); toast();
};
...
$("#reminders").onclick=e=>{
  ...
  if(action.dataset.action==="toggle"){item.done=!item.done;saveStore();render()}
  ...
  if(action.dataset.action==="delete"){reminders=reminders.filter(r=>r.id!==id);saveStore();render()}
};
